...
namespace Zian.NpoiCore.Mapper
{
    ...
    public static class AnonymousTypeFactory
    {
        ...
        {
            var typeDictionary = valueDictionary.ToDictionary(kv => kv.Key,
                kv => kv.Value != null ? kv.Value.GetType() : typeof(object));
        ...
        public static Type CreateType(IEnumerable<KeyValuePair<string, Type>> typePairs, bool isMutable, Type parent)
        {
            ...
            var genericTypeDefinition =
                GetOrCreateGenericTypeDefinition(propertyNames.ToList(), isMutable: isMutable, parent: parent);
            ...
            Type parent)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(propertyNames), propertyNames.Count,
                    "At least one property name is required to create an anonymous type");
            }
        ...
        private static string GenerateGenericTypeDefinitionName(ICollection<string> propertyNames, bool isMutable,
            Type parent)
        {
        ...
        private static Type CreateGenericTypeDefinitionNoLock(string genericTypeDefinitionName,
            ICollection<string> propertyNames, bool isMutable, Type parent)
        {
            ...
            var typeParameterPairs = propertyNames.Zip(typeParameters,
                (propertyName, typeParameter) =>
                    new KeyValuePair<string, GenericTypeParameterBuilder>(propertyName, typeParameter)
            ).ToArray();
            ...
            foreach (var pair in typeParameterPairs)
            {
                ...
                var property = typeBuilder.DefineProperty(propertyName, PropertyAttributes.None, typeParameter,
                    Type.EmptyTypes);
                ...
                if (isMutable)
                {
                    var setMethodBuilder = typeBuilder.DefineMethod(
                        ...
                        attributes: MethodAttributes.PrivateScope | MethodAttributes.Public |
        ...
        private static void DefineEqualsMethod(TypeBuilder typeBuilder, ICollection<FieldBuilder> fields)
        {
            ...
            foreach (var field in fields)
            {
                ...
                var equalityComparerEqualsMethod = TypeBuilder.GetMethod(equalityComparerType,
                    equalityComparerEqualsGenericMethodDefinition);
                var equalityComparerDefaultPropertyGetter = TypeBuilder.GetMethod(equalityComparerType,
                    equalityComparerDefaultGenericPropertyGetterDefinition);
            ...
            typeBuilder.DefineMethodOverride(equalsMethodBuilder,
        ...
        private static void DefineGetHashCodeMethod(TypeBuilder typeBuilder, IEnumerable<FieldBuilder> fields)
        {
            ...
            foreach (var field in fieldBuilders)
            {
                ...
                var equalityComparerDefaultPropertyGetter = TypeBuilder.GetMethod(equalityComparerType,
                    equalityComparerDefaultGenericPropertyGetterDefinition);
                var equalityComparerGetHashCodeMethod = TypeBuilder.GetMethod(equalityComparerType,
                    equalityComparerGetHashCodeGenericMethodDefinition);
        ...
        private static void DefineToStringMethod(TypeBuilder typeBuilder,
            IEnumerable<KeyValuePair<string, FieldBuilder>> fieldPairs)
        {
